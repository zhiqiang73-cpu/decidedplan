...
| 指标 | 门槛 | 说明 |
|------|------|------|
| OOS 胜率（扣费后） | >= 65% | 扣除 Maker 0.04% 来回后的净胜率（vs_entry 智能出场计算，不得用固定K线持仓） |
| P(MFE > MAE) | >= 65% | **核心入场质量门槛**：入场后价格朝正确方向走的概率。做多=上涨幅度>下跌幅度，做空=下跌幅度>上涨幅度。这就是"猜对方向的概率"。原 MFE_cov@0.04% 已废弃，BTC随便几根K线就走0.04%，对任何入场都能达到90%+，无法区分好坏信号。 |
